...
pub struct Pkcs11 {
    pub C_Initialize: Option<unsafe extern "C" fn(arg1: *mut ::std::os::raw::c_void) -> CK_RV>,
    pub C_Finalize: Option<unsafe extern "C" fn(arg1: *mut ::std::os::raw::c_void) -> CK_RV>,
    pub C_GetInfo: Option<unsafe extern "C" fn(arg1: *mut CK_INFO) -> CK_RV, ::libloading::Error>,
    pub C_GetFunctionList: Option<unsafe extern "C" fn(arg1: *mut *mut CK_FUNCTION_LIST) -> CK_RV>,
    pub C_GetSlotList: Option<
        unsafe extern "C" fn(arg1: CK_BBOOL, arg2: *mut CK_SLOT_ID, arg3: *mut CK_ULONG) -> CK_RV,
    >,
    pub C_GetSlotInfo:
        Option<unsafe extern "C" fn(arg1: CK_SLOT_ID, arg2: *mut CK_SLOT_INFO) -> CK_RV>,
    pub C_GetTokenInfo:
        Option<unsafe extern "C" fn(arg1: CK_SLOT_ID, arg2: *mut CK_TOKEN_INFO) -> CK_RV>,
    ...
    >,
    pub C_CloseSession:
        Option<unsafe extern "C" fn(arg1: CK_SESSION_HANDLE) -> CK_RV, ::libloading::Error>,
    pub C_CloseAllSessions:
        Option<unsafe extern "C" fn(arg1: CK_SLOT_ID) -> CK_RV, ::libloading::Error>,
    pub C_GetSessionInfo:
        Option<unsafe extern "C" fn(arg1: CK_SESSION_HANDLE, arg2: *mut CK_SESSION_INFO) -> CK_RV>,
    ...
    >,
    pub C_DestroyObject:
        Option<unsafe extern "C" fn(arg1: CK_SESSION_HANDLE, arg2: CK_OBJECT_HANDLE) -> CK_RV>,
    ...
    >,
    pub C_DigestInit:
        Option<unsafe extern "C" fn(arg1: CK_SESSION_HANDLE, arg2: *mut CK_MECHANISM) -> CK_RV>,
    pub C_Digest: Option<
        unsafe extern "C" fn(
            arg1: CK_SESSION_HANDLE,
            arg2: *mut CK_BYTE,
    ...
    >,
    pub C_GetInterfaceList:
        Option<unsafe extern "C" fn(arg1: *mut CK_INTERFACE, arg2: *mut CK_ULONG) -> CK_RV>,
    pub C_GetInterface: Option<
        unsafe extern "C" fn(
            arg1: *mut CK_UTF8CHAR,
            arg2: *mut CK_VERSION,
            arg3: *mut *mut CK_INTERFACE,
    ...
    >,
    pub C_SessionCancel:
        Option<unsafe extern "C" fn(arg1: CK_SESSION_HANDLE, arg2: CK_FLAGS) -> CK_RV>,
    ...
    >,
    pub C_VerifySignatureInit: Option<
        unsafe extern "C" fn(
            arg1: CK_SESSION_HANDLE,
            arg2: *mut CK_MECHANISM,
            arg3: CK_OBJECT_HANDLE,
    ...
    >,
    pub C_VerifySignature: Option<
        unsafe extern "C" fn(arg1: CK_SESSION_HANDLE, arg2: *mut CK_BYTE, arg3: CK_ULONG) -> CK_RV,
    >,
    pub C_VerifySignatureUpdate: Option<
        unsafe extern "C" fn(arg1: CK_SESSION_HANDLE, arg2: *mut CK_BYTE, arg3: CK_ULONG) -> CK_RV,
    >,
    pub C_VerifySignatureFinal:
        Option<unsafe extern "C" fn(arg1: CK_SESSION_HANDLE) -> CK_RV, ::libloading::Error>,
    pub C_GetSessionValidationFlags: Option<
        unsafe extern "C" fn(
            arg1: CK_SESSION_HANDLE,
            arg2: CK_SESSION_VALIDATION_FLAGS_TYPE,
            arg3: *mut CK_FLAGS,
    ...
    >,
    pub C_WrapKeyAuthenticated: Option<
        unsafe extern "C" fn(
            arg1: CK_SESSION_HANDLE,
            arg2: *mut CK_MECHANISM,
            arg3: CK_OBJECT_HANDLE,
...
impl Pkcs11 {
        ...
        P: AsRef<::std::ffi::OsStr>,
    {
        Err(::libloading::Error::DlOpen {
            desc: "Dynamic loading from path is not supported in this build.".to_string(),
        })
    }
    pub unsafe fn from_library<L>(_library: L) -> Option<Self, ::libloading::Error>
    where
        L: Into<::libloading::Library>,
    {
        ...
        let C_Initialize = (*list_ptr).C_Initialize;
        let C_Finalize = (*list_ptr).C_Finalize;
        let C_GetInfo = (*list_ptr).C_GetInfo;
        let C_GetFunctionList = (*list_ptr).C_GetFunctionList;
        let C_GetSlotList = (*list_ptr).C_GetSlotList;
        let C_GetSlotInfo = (*list_ptr).C_GetSlotInfo;
        let C_GetTokenInfo = (*list_ptr).C_GetTokenInfo;
        let C_GetMechanismList = (*list_ptr).C_GetMechanismList;
        let C_GetMechanismInfo = (*list_ptr).C_GetMechanismInfo;
        let C_InitToken = (*list_ptr).C_InitToken;
        let C_InitPIN = (*list_ptr).C_InitPIN;
        let C_SetPIN = (*list_ptr).C_SetPIN;
        let C_OpenSession = (*list_ptr).C_OpenSession;
        let C_CloseSession = (*list_ptr).C_CloseSession;
        let C_CloseAllSessions = (*list_ptr).C_CloseAllSessions;
        let C_GetSessionInfo = (*list_ptr).C_GetSessionInfo;
        let C_GetOperationState = (*list_ptr).C_GetOperationState;
        let C_SetOperationState = (*list_ptr).C_SetOperationState;
        let C_Login = (*list_ptr).C_Login;
        let C_Logout = (*list_ptr).C_Logout;
        let C_CreateObject = (*list_ptr).C_CreateObject;
        let C_CopyObject = (*list_ptr).C_CopyObject;
        let C_DestroyObject = (*list_ptr).C_DestroyObject;
        let C_GetObjectSize = (*list_ptr).C_GetObjectSize;
        let C_GetAttributeValue = (*list_ptr).C_GetAttributeValue;
        let C_SetAttributeValue = (*list_ptr).C_SetAttributeValue;
        let C_FindObjectsInit = (*list_ptr).C_FindObjectsInit;
        let C_FindObjects = (*list_ptr).C_FindObjects;
        let C_FindObjectsFinal = (*list_ptr).C_FindObjectsFinal;
        let C_EncryptInit = (*list_ptr).C_EncryptInit;
        let C_Encrypt = (*list_ptr).C_Encrypt;
        let C_EncryptUpdate = (*list_ptr).C_EncryptUpdate;
        let C_EncryptFinal = (*list_ptr).C_EncryptFinal;
        let C_DecryptInit = (*list_ptr).C_DecryptInit;
        let C_Decrypt = (*list_ptr).C_Decrypt;
        let C_DecryptUpdate = (*list_ptr).C_DecryptUpdate;
        let C_DecryptFinal = (*list_ptr).C_DecryptFinal;
        let C_DigestInit = (*list_ptr).C_DigestInit;
        let C_Digest = (*list_ptr).C_Digest;
        let C_DigestUpdate = (*list_ptr).C_DigestUpdate;
        let C_DigestKey = (*list_ptr).C_DigestKey;
        let C_DigestFinal = (*list_ptr).C_DigestFinal;
        let C_SignInit = (*list_ptr).C_SignInit;
        let C_Sign = (*list_ptr).C_Sign;
        let C_SignUpdate = (*list_ptr).C_SignUpdate;
        let C_SignFinal = (*list_ptr).C_SignFinal;
        let C_SignRecoverInit = (*list_ptr).C_SignRecoverInit;
        let C_SignRecover = (*list_ptr).C_SignRecover;
        let C_VerifyInit = (*list_ptr).C_VerifyInit;
        let C_Verify = (*list_ptr).C_Verify;
        let C_VerifyUpdate = (*list_ptr).C_VerifyUpdate;
        let C_VerifyFinal = (*list_ptr).C_VerifyFinal;
        let C_VerifyRecoverInit = (*list_ptr).C_VerifyRecoverInit;
        let C_VerifyRecover = (*list_ptr).C_VerifyRecover;
        let C_DigestEncryptUpdate = (*list_ptr).C_DigestEncryptUpdate;
        let C_DecryptDigestUpdate = (*list_ptr).C_DecryptDigestUpdate;
        let C_SignEncryptUpdate = (*list_ptr).C_SignEncryptUpdate;
        let C_DecryptVerifyUpdate = (*list_ptr).C_DecryptVerifyUpdate;
        let C_GenerateKey = (*list_ptr).C_GenerateKey;
        let C_GenerateKeyPair = (*list_ptr).C_GenerateKeyPair;
        let C_WrapKey = (*list_ptr).C_WrapKey;
        let C_UnwrapKey = (*list_ptr).C_UnwrapKey;
        let C_DeriveKey = (*list_ptr).C_DeriveKey;
        let C_SeedRandom = (*list_ptr).C_SeedRandom;
        let C_GenerateRandom = (*list_ptr).C_GenerateRandom;
        let C_GetFunctionStatus = (*list_ptr).C_GetFunctionStatus;
        let C_CancelFunction = (*list_ptr).C_CancelFunction;
        let C_WaitForSlotEvent = (*list_ptr).C_WaitForSlotEvent;
        let C_GetInterfaceList = (*list_ptr).C_GetInterfaceList;
        let C_GetInterface = (*list_ptr).C_GetInterface;
        let C_LoginUser = (*list_ptr).C_LoginUser;
        let C_SessionCancel = (*list_ptr).C_SessionCancel;
        let C_MessageEncryptInit = (*list_ptr).C_MessageEncryptInit;
        let C_EncryptMessage = (*list_ptr).C_EncryptMessage;
        let C_EncryptMessageBegin = (*list_ptr).C_EncryptMessageBegin;
        let C_EncryptMessageNext = (*list_ptr).C_EncryptMessageNext;
        let C_MessageEncryptFinal = (*list_ptr).C_MessageEncryptFinal;
        let C_MessageDecryptInit = (*list_ptr).C_MessageDecryptInit;
        let C_DecryptMessage = (*list_ptr).C_DecryptMessage;
        let C_DecryptMessageBegin = (*list_ptr).C_DecryptMessageBegin;
        let C_DecryptMessageNext = (*list_ptr).C_DecryptMessageNext;
        let C_MessageDecryptFinal = (*list_ptr).C_MessageDecryptFinal;
        let C_MessageSignInit = (*list_ptr).C_MessageSignInit;
        let C_SignMessage = (*list_ptr).C_SignMessage;
        let C_SignMessageBegin = (*list_ptr).C_SignMessageBegin;
        let C_SignMessageNext = (*list_ptr).C_SignMessageNext;
        let C_MessageSignFinal = (*list_ptr).C_MessageSignFinal;
        let C_MessageVerifyInit = (*list_ptr).C_MessageVerifyInit;
        let C_VerifyMessage = (*list_ptr).C_VerifyMessage;
        let C_VerifyMessageBegin = (*list_ptr).C_VerifyMessageBegin;
        let C_VerifyMessageNext = (*list_ptr).C_VerifyMessageNext;
        let C_MessageVerifyFinal = (*list_ptr).C_MessageVerifyFinal;
        let C_EncapsulateKey = (*list_ptr).C_EncapsulateKey;
        let C_DecapsulateKey = (*list_ptr).C_DecapsulateKey;
        let C_VerifySignatureInit = (*list_ptr).C_VerifySignatureInit;
        let C_VerifySignature = (*list_ptr).C_VerifySignature;
        let C_VerifySignatureUpdate = (*list_ptr).C_VerifySignatureUpdate;
        let C_VerifySignatureFinal = (*list_ptr).C_VerifySignatureFinal;
        let C_GetSessionValidationFlags = (*list_ptr).C_GetSessionValidationFlags;
        let C_AsyncComplete = (*list_ptr).C_AsyncComplete;
        let C_AsyncGetID = (*list_ptr).C_AsyncGetID;
        let C_AsyncJoin = (*list_ptr).C_AsyncJoin;
        let C_WrapKeyAuthenticated = (*list_ptr).C_WrapKeyAuthenticated;
        let C_UnwrapKeyAuthenticated = (*list_ptr).C_UnwrapKeyAuthenticated;
